...
def test_infer_realtime_prefix_transform_supports_quaternion_relative_rotation():
    franka_inputs = franka_policy.FrankaInputs(
        model_type=_model.ModelType.PI05,
        state_dim=8,
    )
    delta_xyz = _transforms.DeltaActions(_transforms.make_bool_mask(3, -5))
    delta_quat = _transforms.DeltaQuaternionActions(quat_start=3)
    norm = _transforms.Normalize(
        {
            "state": _normalize.NormStats(mean=np.linspace(0.1, 0.8, 8), std=np.linspace(1.1, 1.8, 8)),
            "actions": _normalize.NormStats(mean=np.linspace(-0.5, 0.2, 8), std=np.linspace(1.5, 2.2, 8)),
        }
    )
    pad = _transforms.PadStatesAndActions(32)
    ...
    policy = _policy.Policy(
        model,
        transforms=[franka_inputs, delta_xyz, delta_quat, norm, pad],
        is_pytorch=True,
    )
    ...
    state_quat = franka_inputs(_make_obs(obs_state.copy()))["state"]
    expected_prefix = action_prefix.copy()
    expected_prefix = delta_xyz({"state": state_quat.copy(), "actions": expected_prefix})["actions"]
    expected_prefix = delta_quat({"state": state_quat.copy(), "actions": expected_prefix})["actions"]
    expected_prefix = norm({"actions": expected_prefix})["actions"]
    expected_prefix = pad({"state": state_quat.copy(), "actions": expected_prefix})["actions"]
