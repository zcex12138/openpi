...
class DualCameraProvider:
    ...
    def __init__(
        ...
        *,
        ...
        if d400_kind == "realsense":
            ...
            self._d400_is_bgr = True  # Xense returns BGR
        else:
    ...
    def get_frames(self) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        d400_frame = self._d400.get_frame()  # BGR/RGB or (RGB, marker3d)
        d400_img = d400_frame[0] if isinstance(d400_frame, tuple) else d400_frame
...
class MultiCameraProvider:
    """Camera provider with L500, D400, and multiple Xense cameras."""
    ...
    def __init__(
        ...
        *,
        ...
        xense_frame_keys: list[str],
        xense_marker_keys: list[str],
        l500_kwargs: dict[str, Any],
        d400_kwargs: dict[str, Any],
        convert_bgr: bool,
        l500_key: str,
        ...
        self._xense_frame_keys = xense_frame_keys
        self._xense_marker_keys = xense_marker_keys
        self._l500_key = l500_key
        self._d400_key = d400_key
        ...
        if d400_kind == "realsense":
            ...
            if not xense_indices:
                raise ValueError("xense_indices is required when d400_kind='xense'")
            self._d400 = SimpleXenseCamera(camera_index=xense_indices[0])
            self._d400_is_bgr = True
        else:
            raise ValueError(f"Unsupported d400 kind: {d400_kind}")

        self._xense_cameras = [SimpleXenseCamera(camera_index=idx) for idx in xense_indices]
    ...
    def get_frames(self) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        l500_img = self._l500.get_frame()
        d400_frame = self._d400.get_frame()
        d400_img = d400_frame[0] if isinstance(d400_frame, tuple) else d400_frame
            ...
            while True:
                ...
                try:
                    if server.poll_hz > 0:
                        ...
                        if frames is None:
                            result = server.provider.get_frames()
                            if isinstance(result, tuple) and len(result) == 2:
                                frames, marker3d = result
                            else:
                                frames, marker3d = result, {}
